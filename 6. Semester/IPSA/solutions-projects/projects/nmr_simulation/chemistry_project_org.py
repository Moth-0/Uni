...
def flat(lists):
    '''Flaten a list of lists, ex. [[1,2],[3,4]] -> [1,2,3,4]'''
...
def split_comps(atoms):  # Are atoms in 'sorted' order wrt Seq_ID ?
    ...
    comps = {seq: [] for seq in seq_ids}
    for atom in atoms:
        comps[key(atom)].append(atom)
    return list(comps.values())


def comp_peaks(comp, couplings, input_MHz): 
    '''Compute all peaks for a given amino acid'''

    amino = comp[0]['Comp_ID']
    print(f'{amino = }')
    atoms = {atom['Atom_ID']: atom for atom in comp}
    peak = {atom_id: atom_peak(amino, atom_id, input_MHz * float(atom['Val']))
    ...
    for B_id in atoms:
        As = []
        if B_id in couplings[amino]:
            for A_id, J in couplings[amino][B_id].items():
                if A_id in atoms:
                    As.append((peak[A_id], J))
        peaks.extend(apply_couplings(reversed(As), peak[B_id]))
    
    return peaks


def protein_peaks(atoms, couplings, input_MHz):
    comps = split_comps(atoms)
    return flat(comp_peaks(comp, couplings, input_MHz) for comp in comps)
    ...
    with open(coupling_file, 'r', newline='') as file:
        ...
        for row in csv_in:
            ...
            if atom2 in couplings[amino][atom1]:
                print('WARNING: couplings (%s, %s) already defined for %s' % (atom1, atom2, amino))
...
def csv_extract(file_name_in, file_name_out, conditions):
    ...
    filters = " ".join('%s=%s' % (column, value)
                       for column, value in conditions.items())
    print('Extracting %s -> %s (%s)' % (file_name_in, file_name_out, filters))
    ...
    with open(file_name_in, 'r', newline='') as file_in:
        with open(file_name_out, 'w', newline='') as file_out:
            ...
            for column in set(conditions) - set(header):
                print('WARNING: Column %s does not exist' % column)
    ...
    print('Duration: %.1f seconds, %s lines selected' % (end - start, count))
...
def plot_two_coupled_peaks():
    ...
    plt.plot(X, eval_peak(X, A), "C0:", label="A = %s" % str(A))
    plt.plot(X, eval_peak(X, B), "C1:", label="B = %s" % str(B))
    ...
    plt.xlabel("Hz")
    plt.legend(title="Coupling J$_{\mathrm{AB}}$=10")
...
def plot_three_coupled_peaks():
    ...
    plt.plot(X, eval_peak(X, A1), "C0:", label="A$_1$ = %s" % str(A1))
    plt.plot(X, eval_peak(X, A2), "C1:", label="A$_2$ = %s" % str(A2))
    plt.plot(X, eval_peak(X, B), "C2:", label="B = %s" % str(B))
    ...
    plt.legend(title="Coupling J$_{\mathrm{A}_1\mathrm{B}}$=%s J$_{\mathrm{A}_2\mathrm{B}}$=%s" % (J_A1, J_A2))
...
def plot_molecule(csv_file, title, input_MHz):
    ...
    plt.legend(title="%s (%g MHz)" % (title, input_MHz))
...
NFGAIL_atoms = read_molecule("NFGAIL.csv")
...
plot_molecule("NFGAIL.csv", "Peptide NFGAIL", 400.13)
plt.subplot2grid((3, 1), (1, 0))
plot_molecule("68_ubiquitin.csv", "Ubiquitin", 500)
plt.subplot2grid((3, 1), (2, 0))
plot_molecule("entry6203.csv", "ThrB12-DKP-insulin", 500)
