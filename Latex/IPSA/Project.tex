...
\chapter{Code}
Thoughout the code I have used the decorator \code{@time_it}, that we have from the decorators lecture, to time the functions used. 
...
This is done with the json library. By doing this the weights and biases from MNIST can be visualized, as seen in figure \ref{fig:MNIST}.
\begin{figure}[H]
    \centering
...
For these I wrote assertation tests to check if the given vectors are the same length, and that the scalar is actually a number. 
These test are the main test that run throughout the code, and have helped if there were problems with the format of the weights matrix. 
...
The \code{evaluate} functionn also calculates, which calculates the predictions and cost, also calculates the accuracy,
which is the number of correct guesses divided by the total number of guesses.
...
for this the \code{create_batches} function is used, which uses the \code{random} library to shuffel the images. 
...
\end{align*}
where $σ$ is the step length which is set to $0.1$. Notice the -= because I did not, and i spend a lot of time not understanding why thing didn't work. 
...
After that is the actual training of the network in the \code{learn} function. Here I first create a uniformly random network, 
I then run it for an amount of epochs, where in each epoch the training set is split up into random batches of a set size. 
For every batch i run the \code{update} function, i then make a save the weights matrix and evaluation of the network, to be used in animations. 
For each epoch the time taken is recorded, and the network is evaluated on the full test set, and saved using \code{lineal_save} if it is more accurate then the last. 
...
In figure \ref{fig:A} the traines weights matrix is shown. We can see a similar style to the MNIST weights, exept here we have more possitive weights. 
The accuracy is calculated after each batch, and after 3rd batch the accuracy maxes out at $85.3\%$ \textcolor{red}{cost}. 
We can also see the cost and accuracy as a function uf number of updates in figure \ref{fig:cost}. 

The animations have been uploaded to brightspace and can be seen here \url{Here}. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Billeder/cost_long.png}
...
To make the grahps I had to append a copy of every weight matrix to a list, and evaluate the network after every batch. 
This added a lot of execution time to the code, created an option to not save this data, and only evaluated on 300 random images. 
...
I also tried to play with the batch size, and found that a size of 2000 would runt the full function in 1 minute per epoch, but the accuracy only reaches around $83\%$ at 5 epochs. 
