...
\begin{itemize}
    ...
    \item \textbf{Chemical Potential: }
    \begin{align*}
        μ &= -kT\ln\br{\frac{VZ_{int}}{Nv_q}}, \eq{6.93}\\
        v_Q &= \br{\frac{h}{\sqrt{2\pi mkT}}}^3.
    \end{align*}
    For monatomic gas $Z_{int} = 1$. 
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Grand Partition Function ($\mathcal{Z}$):} The normalization constant, also called the Gibbs sum for fermions.
    \begin{align*}
        \mathcal{Z} = \sum_s e^{-[E(s) - \mu N(s)]/kT} \eq{7.7}
    \end{align*}
    The sum is over all possible microstates, including all possible values of $N(s)$. For \textbf{Bosons} it looks like 
    \begin{align*}
        \mathcal{Z} = \prod_{k}\br{\sum_{n=0}^{\infty}\e{-\frac{n\br{E_k-μ}}{k_bT}}}.
    \end{align*}
    ...
    \item \textbf{Avergae Number of Particles in a System: } When using the Grand Partition Function: 
    \begin{align*}
        \bar{N} = \frac{k_bT}{\mathcal{Z}}\frac{\partial \mathcal{Z}}{\partial μ}
    \end{align*}
\end{itemize}
